...
import type { PlatformWorkflow, Tenant } from "@/lib/platform-types";
...
function TableBody({ workflows, tenants }: { workflows: PlatformWorkflow[]; tenants: Tenant[] }) {
  ...
  }, [workflows, tenantFilter, status, q]);

  const tenantName =
    tenantFilter === "all"
      ? null
      : tenants.find((t) => t.id === tenantFilter)?.name ?? tenantFilter;
...
export function WorkflowsTable({ workflows, tenants }: { workflows: PlatformWorkflow[]; tenants: Tenant[] }) {
  return (
    <PlatformShell
      title="Workflows"
    ...
    >
      <Suspense fallback={<div className="p-8 text-sm text-ink-500">Loading…</div>}>
        <TableBody workflows={workflows} tenants={tenants} />
      </Suspense>
    </PlatformShell>
  );
}
