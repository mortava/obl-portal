...
import { acknowledgeAlertAction } from "@/app/platform/alerts/actions";
...
export function AlertsList({ alerts }: { alerts: PlatformAlert[] }) {
...
function AlertCard({ alert, muted }: { alert: PlatformAlert; muted?: boolean }) {
  ...
  return (
    <div className={cls("card p-4 border-l-4", meta.ring, muted && "opacity-60")}>
      <div className="flex items-start gap-3">
        ...
        {!muted && (
          <button
            onClick={() => start(() => acknowledgeAlertAction(alert.id))}
            disabled={pending}
